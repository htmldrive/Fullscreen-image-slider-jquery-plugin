(function($) {
  ...
  fullscreen_slider.prototype.init = function(node, settings) {
    ...
    caller.chlid_count = $(node).children('.fullscreen_slider_child').length;
    caller.node = $(node);
  ...
  fullscreen_slider.prototype.reset = function() {
    ...
    current_child.css({"top": caller.padding, "left": (window_width - image_width)/2}).show();
    ...
    prev_child.css({"top": (window_height-image_height)/2, "left": prev_left}).show();
    ...
    next_child.css({"top": (window_height-image_height)/2, "left": next_left}).show();
    ...
    first_child.css({"top": (window_height-image_height)/2, "left": first_left}).show();
    ...
    last_child.css({"top": (window_height-image_height)/2, "left": last_left}).show();

    node.children('.fullscreen_slider_child').children('.fullscreen_slider_caption').hide();
    setTimeout(function(){current_child.children('.fullscreen_slider_caption').show();},100);
  ...
  fullscreen_slider.prototype.play = function(dom) {
    ...
    if(dom.index() >= caller.current){
      if(caller.current < caller.chlid_count-1){
        caller.forward();
      }
    }else{
      caller.backward();
